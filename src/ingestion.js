...
const { fetchRepository, cleanupRepository } = require('./githubFetcher');
...
async function analyzeRepo(repoUrl) {
    ...
    try {
        // Step 1: Fetch repository using GitHub API
        console.log(`[1/4] Fetching repository via GitHub API...`);
        const fetchResult = await fetchRepository(repoUrl);
        repoPath = fetchResult.path;
        fetchStats = fetchResult.stats;

        // Step 2: Detect languages
        console.log(`[2/4] Detecting languages...`);
        const languageStats = await detectLanguages(repoPath);

        // Step 3: Analyze project-level metrics
        console.log(`[3/4] Analyzing project structure...`);
        const projectMetrics = await analyzeProject(repoPath, repoUrl);

        // Step 4: Run language-specific analyzers
        console.log(`[4/4] Running code analyzers...`);
        const codeAnalysis = await runAnalyzers(repoPath, languageStats);

        // Compile final report
        const report = compileReport(repoUrl, languageStats, projectMetrics, codeAnalysis, fetchStats);

        return report;

    } finally {
        // Cleanup: Remove fetched repo
        if (repoPath) {
...
function compileReport(repoUrl, languageStats, projectMetrics, codeAnalysis, fetchStats) {
    ...
    // Add fetch statistics if available
    if (fetchStats) {
        report.fetchStats = {
            filesAnalyzed: fetchStats.downloaded,
            fetchTimeSeconds: fetchStats.elapsedSeconds,
